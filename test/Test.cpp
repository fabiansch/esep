...
void Test::actuatorsTestHelper(hardwareLayer::HardwareLayer* hal, Signal& signal) {
	switch (signal.name) {
		// mmi
		// traffic lights
		...
		case Signalname::SWITCH_CLOSE:
			...
			break;
		default:
			LOG_WARNING<<__FUNCTION__<<" does not support following Signal: "<<(int)signal.name<<endl;
			break;
	}
}
