...
private:
	struct State { //top-level state
		...
		void calibrationFailed(string functionname) {
			cout << "Calibration failed: " << name() << ", caused of "
					<< functionname << endl;
			new (this) FAIL_STATE;
		}
		...
		std::string name() const {
			...
			struct WaitingForItem: public State {
				WaitingForItem() {
					...
					cout << "height_conveyor_belt: " << height_conveyor_belt
							<< endl;
				}
				virtual void lb_input_interrupted() {
			...
			struct ArrivalAtInput: public State {
				ArrivalAtInput() {
					cout << "=============== ArrivalAtInput =================="
							<< endl;
					if (cb_this != cb_first) {
						duration<float> durationfs = steady_clock::now()
								- *timeFrameStart;
						milliseconds durationMS = duration_cast < milliseconds
								> (durationfs);
						unsigned int time = durationMS.count();
						time_output_to_input = time;
						cout << "time_output_to_input: " << time_output_to_input
								<< endl;
					}
					hal->motorRotateClockwise();
					hal->motorFast();
			...
			struct ArrivalAtHeight: public State {
				ArrivalAtHeight() {
					...
					cout << "time_input_to_height: " << time_input_to_height
							<< endl;
			...
			struct ArrivalSwitch: public State {
				ArrivalSwitch() {
					cout << "=============== ArrivalSwitch =================="
							<< endl;
					std::chrono::duration<float> durationfs =
							steady_clock::now() - *timeFrameStart;
					std::chrono::milliseconds durationMS =
							std::chrono::duration_cast
									< std::chrono::milliseconds > (durationfs);
					unsigned int time = durationMS.count();
					time_height_to_switch = time;
					cout << "time_height_to_switch: " << time_height_to_switch
							<< endl;
			...
			struct ArrivalOutput: public State {
				ArrivalOutput() {
					...
					milliseconds durationMS = duration_cast < milliseconds
							> (durationfs);
					time_switch_to_output = durationMS.count();
					cout << "time_switch_to_output: " << time_switch_to_output
							<< endl;

					if (cb_this == cb_last) {
						hal->motorStop();
						cout
								<< "Item arrived last CB's output.\nPlease put one Item on input of each conveyor belt. "
								<< endl;
					}
					if (cb_this != cb_last) {
						hal->sendSerial(
								Signal(cb_this, cb_next,
										Signalname::SERIAL_TRANSFER_ITEM));
						cout
								<< "Calibration active on next conveyor belt. Please wait for instructions"
								<< endl;
					}
				}
				virtual void lb_output_freed() {
